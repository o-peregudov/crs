//*********************************************************************//
//* Module name: crcstuff.c                                           *//
//* Date: 27 Dec 87 (modified Mar 2003)                               *//
//* Environment: Turbo C 1.0                                          *//
//* Author: R. E. Faith (modified by O. N. Peregudov)                 *//
...
static unsigned short int crc_ccitt_tab[] = {
...
// are sent MSB to LSB.
unsigned short int get_crc_ccitt( unsigned short int crc, const char * b, const int sz )
{
	int i;
      for( i = 0; i < sz; ++i )
            crc = (crc << 8) ^ crc_ccitt_tab[ ( (crc >> 8) ^ b[i] ) & 0xFF ];
      return crc;
}
